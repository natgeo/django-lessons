...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field prior_activities on 'Lesson'
        db.delete_table('curricula_lesson_prior_activities')

        # Adding M2M table for field prior_lessons on 'Lesson'
        db.create_table('curricula_lesson_prior_lessons', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('from_lesson', models.ForeignKey(orm['curricula.lesson'], null=False)),
            ('to_lesson', models.ForeignKey(orm['curricula.lesson'], null=False))
        ))
        db.create_unique('curricula_lesson_prior_lessons', ['from_lesson_id', 'to_lesson_id'])

    def backwards(self, orm):
        # Removing M2M table for field prior_lessons on 'Lesson'
        db.delete_table('curricula_lesson_prior_lessons')

        # Adding M2M table for field prior_activities on 'Lesson'
        db.create_table('curricula_lesson_prior_activities', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('lesson', models.ForeignKey(orm['curricula.lesson'], null=False)),
            ('activity', models.ForeignKey(orm['curricula.activity'], null=False))
        ))
        db.create_unique('curricula_lesson_prior_activities', ['lesson_id', 'activity_id'])
